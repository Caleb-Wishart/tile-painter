...
local curved_rail_mask = require("scripts.util.curved-rail")
...
local function get_entity_tiles(player, entity, delta)
    ...
    for i = 1, #search_boxes do
        ...
        if sbox.orientation ~= flib_orientation.north and sbox.orientation ~= flib_orientation.east
            and sbox.orientation ~= flib_orientation.south and sbox.orientation ~= flib_orientation.west then
...
function tp_painter.paint_polygon(player, tdata, whatIf)
    ...
    if n == 1 then
        ...
        for _, tile in pairs(area) do
            local pos = flib_position.ensure_explicit(tile.position) --[[@as MapPosition]]
            pos = flib_position.add(pos, { 0.5, 0.5 })
            ---@diagnostic disable-next-line: need-check-nil
            -- All positions are guaranteed non-nil due to explicit call
    ...
    elseif n == 2 then
        ...
        for _, tile in pairs(area) do
            local position = tile.position ---@cast position MapPosition
            if bounding_box.line_intersect_AABB(p1, p2, flib_boundingBox.from_position(position, true)) then
                table.insert(tiles, tile)
            end
        end
    elseif n > 2 then
        -- TODO: find inner BB and exclude from search area to reduce number of tiles to check
        local center = tdata.center ---@cast center - nil
        local vertices = polygon.polygon_vertices(n, r, center, theta)
        for _, tile in pairs(area) do
            local insert = false
            local position = tile.position ---@cast position MapPosition
            if n ~= 2 and tdata.fill and polygon.point_in_polygon(position, n, vertices) then
                insert = true
            end
            for i = 1, n do
                local p1 = vertices[i]                 ---@cast p1 - nil
                local p2 = vertices[i % #vertices + 1] ---@cast p2 - nil
                if bounding_box.line_intersect_AABB(p1, p2, flib_boundingBox.from_position(position, true)) then
                    insert = true
                    break
                end
