...
local function on_import_confirm_click(e, self, tdata, pdata)
    ...
    for _, setting in pairs(import.config) do
        if c == 0 and setting.entity ~= "signal-everything" then
            create_error_text({ "failed-to-import-string", "Invalid Config" })
            return
        end
        if c == 1 and setting.entity ~= "signal-anything" then
            create_error_text({ "failed-to-import-string", "Invalid Config" })
            return
        end
        if setting.entity ~= nil and setting.entity ~= "signal-everything" and setting.entity ~= "signal-anything" then
...
function tp_tab_entity.populate_config_table(self)
    ...
    local function build_row(self, tbl, row)
        ...
        if config_data[row] == nil then
            config_data[row] = {
                entity = (row == 1 and "signal-everything") or (row == 2 and "signal-anything") or nil,
                tile_0 = nil,
                tile_1 = nil,
                tile_2 = nil,
            }
        ...
        if row == 1 then
            flib_gui.add(tbl, {
                ...
                tooltip = { "gui.tp-tooltip-entity-everything" },
                enabled = false,
            })
        elseif row == 2 then
            flib_gui.add(tbl, {
            ...
            flib_gui.add(tbl, {
                ...
                elem_filters = {
                    ...
                    -- Limit rail to specific tab
                    { filter = "rail",          mode = "and", invert = true },
                },
        ...
        local enabled = pdata.whitelist or row == 1 or row == 2
        for i = 1, #tiles do
            local style = "slot_button"
            if tiles[i] == "tile_0" then
                style = "flib_slot_button_orange"
            end
            style = row == 1 and "flib_slot_button_red" or row == 2 and "flib_slot_button_green" or style
            flib_gui.add(tbl, {
                type = "choose-elem-button",
                style = style,
                elem_type = "tile",
