...
local rail = require("scripts.util.rail")
...
function tp_tab_rail.get_config(pdata)
    ...
    if pdata.group_rail then
        ...
        for rial, _ in pairs(rail.base_rails) do
            for _, data in pairs(cfg) do
                if data.rail == rial then
                    goto next
                end
            end
            cfg[#cfg + 1] = {
...
function tp_tab_rail.populate_scroll_pane(self)
    ...
    local function build_row(self, pane, row, data, pdata)
        local function build_table(row, data)
            local tiles = {
                "1-1", "1-2", "1-3", "1-4", "1-5",
                "2-1", "2-2", "2-3", "2-4", "2-5",
                "3-1", "3-2", "3-3", "3-4", "3-5",
        ...
        flib_gui.add(pane, {
            ...
            },
            {
                ...
                {
                    type = "table",
                    name = "tp_rail_config_table_" .. row,
                    column_count = 5,
                    draw_vertical_lines = true,
                    draw_horizontal_lines = true,
                    vertical_centering = true,
                    style = "table",
                    style_mods = { cell_padding = 2 },
                    table.unpack(build_table(row, data)),
                },
                {
                    type = "empty-widget",
                    style = "flib_horizontal_pusher",
