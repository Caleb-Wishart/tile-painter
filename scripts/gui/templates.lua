    ...
    if opts then
        if opts.on_close_handler then
            table.insert(
                ...
                tp_gui_template.frame_action_button(
                    "close_button", "utility/close", { "gui.close-instruction" }, opts.on_close_handler
                )
            )
        end
        if opts.on_pin_handler then
            table.insert(
                elems, #elems + 1,
                tp_gui_template.frame_action_button(
                    "pin_button", "flib_pin_white", { "gui.flib-keep-open" }, opts.on_pin_handler
                )
            )
        end
        if opts.on_titlebar_click_handler then
            elems.handler = { [defines.events.on_gui_click] = opts.on_titlebar_click_handler }
        end
    end
    return elems
end
...
---@field subheading flib.GuiElemDef | flib.GuiElemDef[] Subheading of the tab
---@field contents   flib.GuiElemDef | flib.GuiElemDef[] Contents of the tab
